...
pub const StdIo = struct {
    ...
    /// Get's input from the user but returns the input as an enum
    /// generated by enum options.  
    pub fn strictInput(
        self: *Self,
    ...
    /// Returns true if user inputs 'yes' or 'y', and 
    /// returns false if user inputs 'no' or 'n'.  
    ...
    pub fn getYesOrNo(
        ...
        const user_input = try self.input(prompt, args);
        var fmt_input: [3]u8 = undefined;

        for(0..user_input.len) |i| fmt_input[i] = user_input[i];
        
        if(user_input.len == 0) return error.NoInput
        else for(0..user_input.len) |i| fmt_input[i] = std.ascii.toLower(fmt_input[i]);
            
        const yes = if(user_input.len == 1) "y" else "yes";
        const no = if(user_input.len == 1) "n" else "no";
        
        const input_to_compare = fmt_input[0..user_input.len];
        if(std.mem.eql(u8, input_to_compare, yes)) return true
        else if(std.mem.eql(u8, input_to_compare, no)) return false
